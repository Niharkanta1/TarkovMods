...
async function main() {
    ...
    try {
        ...
        const projectVersion = createProjectVersion(packageJson);
        logger.log("success", `Project version created: ${projectVersion}`);
        ...
        const zipFilePath = path.join(path.dirname(projectDir), `${projectName}_${projectVersion}.zip`);
        ...
        const zipFileInProjectDir = path.join(projectDir, `${projectName}_${projectVersion}.zip`);
        ...
        logger.log("success", `/${path.relative(process.cwd(), path.join(distDir, `${projectName}_${projectVersion}.zip`))}`);
        logger.log("success", "------------------------------------");
        if (!verbose) {
...
function createProjectVersion(packageJson) {
